...
--}}
{{-- MODIFIED: Form now grows to fill the available space in its flex parent and contains its children's overflow. --}}
<form method="POST" action="{{ $isNew ? route('profile.books.store') : route('profile.books.update', $book) }}" class="flex flex-col flex-grow overflow-hidden" enctype="multipart/form-data">
	@csrf
	{{-- Use POST for update as defined in web.php to simplify form --}}
	
	{{-- MODIFIED: This container now has padding and is the designated scrollable area. --}}
	...
	{{-- MODIFIED: Actions are a non-scrolling footer with its own padding, background, and border. --}}
	<div class="modal-action flex-shrink-0 p-6 bg-base-200 border-t border-base-300">
